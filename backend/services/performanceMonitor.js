...
class MemoryMonitor {
  ...
  checkMemoryUsage() {
    ...
    if (heapPercent >= PERFORMANCE_THRESHOLDS.HEAP_CRITICAL || memPercent >= PERFORMANCE_THRESHOLDS.MEMORY_CRITICAL) {
      console.error(`[Performance] CRITICAL: Heap ${heapPercent.toFixed(1)}%, Memory ${memPercent.toFixed(1)}%`);
      this.triggerRestart();
    }
...
const shouldThrottle = () => {
  ...
  return health.memory.percent >= PERFORMANCE_THRESHOLDS.MEMORY_WARNING ||
         health.heap.percent >= PERFORMANCE_THRESHOLDS.HEAP_WARNING ||
         health.cpu.current >= PERFORMANCE_THRESHOLDS.CPU_WARNING;
};
...
const createPerformanceMiddleware = () => {
  return (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      const cpuPercent = (cpuUsage.user + cpuUsage.system) / 1000000 / duration * 100;
      ...
      if (cpuPercent > PERFORMANCE_THRESHOLDS.CPU_CRITICAL) {
        console.warn(`[Performance] High CPU: ${cpuPercent.toFixed(1)}% for ${req.path}`);
      }

      if (shouldThrottle()) {
        console.warn(`[Performance] System under load, throttling active`);
      }
    });
...
const logPerformance = (label, fn) => {
  return async (...args) => {
    ...
    try {
      ...
      console.log(`[Performance] ${label}: ${duration}ms, mem delta: ${formatBytes(memDelta)}`);
