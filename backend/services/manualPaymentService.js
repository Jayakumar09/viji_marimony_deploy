...
const getPaymentDetails = async (paymentId, userId) => {
  ...
  return {
    payment: {
      ...
      paymentProof: payment.paymentProof ? (payment.paymentProof.startsWith('http') ? payment.paymentProof : `${process.env.SERVER_URL || 'https://viji-marimony-deploy-backend.onrender.com'}${payment.paymentProof}`) : null,
...
const getAdminPayments = async (page = 1, limit = 20, status = null) => {
  ...
  return {
    payments: payments.map(p => ({
      ...
      status: p.paymentStatus,
      transactionId: p.transactionId,
      paymentProof: p.paymentProof ? (p.paymentProof.startsWith('http') ? p.paymentProof : `${process.env.SERVER_URL || 'https://viji-marimony-deploy-backend.onrender.com'}${p.paymentProof}`) : null,
      paymentDate: p.paymentDate,
      createdAt: p.createdAt,
      user: p.user
    })),
